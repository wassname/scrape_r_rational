...
* Author: u/lsparrish *
* URL: https://www.reddit.com/r/rational/comments/39x125/d_dyson_sphere_feasibility_vs_shoddy_online/
* Score: 17

* Created: 2015-06-15T14:23:54
...
> **u/SkyTroupe** [+7]  (40 minutes later)
> 
...
>> **u/danielravennest** [+3]  (2 hours later)
>> 
...
>>> **u/danielravennest** [+1]  (2 hours later)
>>> 
...
>>>>>> > I agree with most of your comment, and would invite you to contribute to the wikibooks I have been working on
>>>>>> 
>>>>>> I may take you up on that. However, it occurs to me that my tendency is sometimes more towards original ideas, and/or novel uses for old ones, and original research is something that Wikipedia prohibits (I'm actually skating a thin line with the edits to Dvorsky's page, NOR-wise), while Wikibooks also apparently [frowns on it](https://en.wikibooks.org/wiki/Wikibooks:What_is_Wikibooks#Wikibooks_is_not_a_mirror_or_a_text_repository). So Wikiversity might be a better place to publish/republish certain of these concepts at first. In theory, one could get more eyes on the subject for peer review in this manner (although it looks like WV isn't especially widely known/used).
>>>>>> 
>>>>>> > I was part of a "Solar Power Satellite built from Lunar materials" study sponsored by the Space Studies Institute.
>>>>>> 
>>>>>> Cool! Any chance that's online anywhere with your name credited? I couldn't find it by googling that title. It would strengthen the [case](https://en.wikipedia.org/wiki/User:Lsparrish/Dani_Eder) for giving you a [Wikipedia bio](https://en.wikipedia.org/wiki/Dani_Eder), if you're interested in that.
>>>>>> 
...
>>>>>> 
>>>>>> Then there are the [various kinds](https://en.wikibooks.org/wiki/Space_Transport_and_Engineering_Methods/Guns_and_Accelerators) [of guns](https://en.wikibooks.org/wiki/Space_Transport_and_Engineering_Methods/Guns_and_Accelerators2) which can be mounted on asteroids regardless of where they are -- Stuart Armstrong mentioned mass drivers, which most space nuts know about, but [according to Keith Lofstrom](http://settlement.arc.nasa.gov/Nowicki/SPBI1SI.HTM) these aren't necessarily the cheapest (and probably can't work at all on earth) due to scaling barriers. I'm partial to light gas guns powered by LH2 + solar heating.
>>>>>> 
...
>>>>>>> **u/danielravennest** [+2]  (3 days later)
>>>>>>> 
...
>>>>>>>> **u/lsparrish** [+1]  (7 days later)
>>>>>>>> 
>>>>>>>> > If something is truly new, then yes, go publish it somewhere and link back to it in the wikibook.
>>>>>>>> 
>>>>>>>> Not sure if it's truly new, but I couldn't find [this idea](https://en.wikiversity.org/wiki/User:Lsparrish/Research/Hypervelocity_Landing_Tracks) anywhere else, so I've posted it to wikiversity. Feel free to tear it apart. :)
>>>>>>>> 
>>>>>>>> > If I remember right, it was mainly rare elements not found on the Moon, or difficult to make parts.
>>>>>>>> 
>>>>>>>> Maybe given the [KREEP](https://en.wikipedia.org/wiki/KREEP) regions, we could extract these economically? Of course, early in the boostrapping it still makes sense that we'd want to remain focused on the heavy stuff like iron.
>>>>>>>> 
...
>>>>>>>>> **u/danielravennest** [+2]  (7 days later)
>>>>>>>>> 
...
>>>>> Any and all possible uses you can think of can be met by an incomplete swarm. Biospheres to go camping in? Sure, biomes the size of earth with an average population of "5000 campers and some grad students" are invisible gnats in this context.
>>>>> 
...
>>> **u/ArgentStonecutter** [+0]  *Emergency Mustelid Hologram* (2 hours later)
>>> 
...
>> 
>> [4pi x ((7×(10^7 ) )^2 ) = 6.1575216 x 10^16](https://www.google.com/search?q=4pi+x+%28%287%C3%97%2810%5E7%29%29%5E2%29).
>> 
>> >That leaves us with ~10^9 W.
>> 
>> That's about right, so I guess the above is a typo. But 3.8/6.1 is about 6, so you'd round up to ~10^10 W/km^2. It's [about 9](https://www.google.com/search?q=solar%20irradiance%20mercury) in reality anyway.
>> 
...
>> 
>> ETA: [Stuart Armstrong](https://www.youtube.com/watch?v=zQTfuI-9jIo) assumes the panels will take ten years to place, which is the main bottleneck in his model. I'm rather doubtful that this would be at all necessary, but he did give the disclaimer that it was a very conservative model, and it is the model Dvorsky stuck with.
...
>> 
>> One incarnation of this idea that probably deserves attention due to the sheer level of engineering detail is the [Launch Loop](http://launchloop.com/). A fringe benefit of being a fellow cryonicist living in the same area of the US is that I've met [the author](https://en.wikipedia.org/wiki/Keith_Lofstrom) at various meetups. His other big idea, [Server Sky](http://server-sky.com/) might be more significant, especially in the Dyson Sphere context, since it involves using smart lightweight satellites that maintain a programmed formation with light pressure instead of orbiting in the traditional manner. If we had sats that thin at 0.3 AU, only a very small fraction of Mercury would be needed to fill out the sphere.
>> 
...
>> 
>> Being nothing more than long chunks of steel ballast, they could handle extreme accelerations, so the bouncing at the ends could happen at thousands of gees. The passenger train (itself in an evacuated tube, and lighter than the ballast) would electromagnetically couple with the ballast train to acquire momentum at a lower rate. So you can [calculate](http://keisan.casio.com/exec/system/1224835316) with a 50 gee or 500 m/s^2 acceleration along 100 km over 20 seconds. If you want one that can get humans to orbital velocity, plug in 135 seconds and 60 m/s^2 for acceleration for a ~546 km track.
>> 
>> There's also [lots of other ways](https://en.wikibooks.org/wiki/Space_Transport_and_Engineering_Methods/Guns_and_Accelerators) that could work. Note that even a modest boost can be combined with a rocket to drastically improve its fuel efficiency, since velocity is a square WRT energy.
...
>>>>>> 
>>>>>> This system could be somewhat self-bootstrapping, because you can use it as a track to brake along to get materials to orbital velocity. However, it's not the easiest kind of track of this kind to build, as you could simply take any orbiting mass (say the ISS itself) and attach a trailing cable to it. You need a few hundred km of distance for human compatible acceleration, but bulk materials like we need for bootstrapping purposes can be accelerated much faster, which allows even shorter cables. In either case, you are exploiting [Lenz Law](http://hyperphysics.phy-astr.gsu.edu/hbase/electric/farlaw.html) to create magnetic friction.
>>>>>> 
