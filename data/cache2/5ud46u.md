...
* Author: u/Kuiper *
* URL: https://www.reddit.com/r/rational/comments/5ud46u/d_the_rationality_of_different_litrpg_premises/
* Score: 30

* Created: 2017-02-16T06:04:22
...
> **u/OutOfNiceUsernames** [+15]  *fear of last pages* (4 hours later)
> 
...
>> **u/Subrosian_Smithy** [+6]  *Nudist Beach* (6 hours later)
>> 
...
>> 
>> Have you read [The Gam3](https://thegam3.com/)? In the setting, governments do provide access to special training and farming grounds to their militia and employees.

>>> **u/OutOfNiceUsernames** [+1]  *fear of last pages* (5 days later)
>>> 
>>> I’ve put it for later reading because [it seems to not be finished yet.](https://thegam3.com/table-of-contents/)
>>> 
...
>>> 
>>> Thanks for the recommendation though. Do you maybe know any RPGMech stories that are at least generally rational and contain 300k–500k+ words? So far the only long series that I haven’t tried yet is [*Guardians of the Flame,*](https://www.goodreads.com/series/42300-guardians-of-the-flame) with almost all the rest ending up being a disappointment.

>>>> **u/YetiMarauder** [+2]  (10 days later)
>>>> 
>>>> It's never going to finish the author has dropped it. A real shame too, as it was quite good

> **u/Salaris** [+10]  *Dominion Sorcerer* (12 hours later)
> 
...
>> **u/GaBeRockKing** [+5]  *Horizon Breach: http://archiveofourown.org/works/6785857* (18 hours later)
>> 
>> >Moreover, SAO itself doesn't play like a just-launched MMO, and certainly not like the first one on a new device. No game-breaking bugs? No server crashes? No client-side memory leaks? I don't believe it in the slightest.
>> 
...
>>> **u/Salaris** [+3]  *Dominion Sorcerer* (20 hours later)
>>> 
...
>>> **u/General_Urist** [+1]  (2 days later)
>>> 
...
>>> **u/derefr** [+1]  (15 days later)
>>> 
>>> Oh, like [The Magic Circle](http://store.steampowered.com/app/323380/), sort of. (Though that's the player-character trying to exploit the game to get out.)
>>> 
>>> Or like [Alphaland](http://www.kongregate.com/games/jonaskyratzes/alphaland). (Though that's the *game* exploiting *itself* to extend itself.)
>>> 
>>> Or like the secret ending in [Kaizo Trap](https://www.youtube.com/watch?v=lIES3ii-IOg).

>>>> **u/GaBeRockKing** [+1]  *Horizon Breach: http://archiveofourown.org/works/6785857* (15 days later)
>>>> 
...
>>>>> **u/derefr** [+1]  (15 days later)
>>>>> 
>>>>> Ah, I think I was thinking of [the second one](https://www.youtube.com/watch?v=j9I7SF2ND7I). Treating the game runtime's crashed game state as its own game and using the original game mechanics to advance through it. A bit the [Tron light-cycle escape](http://blog.danielwellman.com/2008/10/real-life-tron-on-an-apple-iigs.html), but with more intentionality.

>> **u/Kuiper** [+2]  (13 hours later)
>> 
...
>>> **u/Salaris** [+1]  *Dominion Sorcerer* (20 hours later)
>>> 
...
>> **u/SpeculativeFiction** [+2]  (7 days later)
>> 
...
>>> **u/Salaris** [+1]  *Dominion Sorcerer* (7 days later)
>>> 
>>> I believe they mentioned later that everyone who was hooked into the machines was moved into hospitals for care. You can see [spoilers for end of season 1](#s "Asuna still in the hospital at the end of the first season.")
>>> 
...
>> > code review
>> 
>> Code review only works for code *checked into* version control. If the "logout lock" logic was embedded in, say, an SQL stored procedure on the production database, where its matching migration was untouched, you'd never find it in code review/QA. Or, more insidiously (yet sadly, more common), if a large part of the game was built in the form of a [business rules engine](https://en.wikipedia.org/wiki/Business_rules_engine), the rules loaded into it would be rendered pretty much opaque to the developers—production business-rules engines are basically "write-only."
>> 
>> > Also, where are all the people from the company that decided to play their own product on launch day?
>> 
>> I agree this part seems off. The only way I can make sense of it is to imagine that nobody even knew they *were* making an MMO, other than the designer. There was some Manhattan Project level shit going on. Like in [*Cube*](https://en.wikipedia.org/wiki/Cube_(film\)).
...
>>> 
>>> Even if somehow it got through devs and QA, which I doubt, I still expect that users would have cracked open the hardware and figured out the potential applications fast. There are videos with people [Opening up an Occulus Rift](https://youtu.be/zfZx_jthHM4) and analyzing the content, for example.
>>> 
>>> > Maybe it's "a new game" in the sense that it's a reused game engine, with its rendering backend newly ported over to VR (like is being done to FFXV presently), and populated with new but not mechanically-novel content (ala Megaman 2-through-5.)
>>> 
>>> Ports to other platforms still are going to have bugs, though. And even companies that have existing hardware capable of handling MMO servers often have terrible launch stability; see the [launch of Diablo 3](http://www.pcmag.com/article2/0,2817,2404481,00.asp), for example. Blizzard has some of the most stable servers and the best teams for "always online" games in the business, but even they have problems. Even the first few *expansions* for WoW had serious problems at launch. 
>>> 
...
> **u/eternal-potato** [+7]  *he who vegetates* (13 hours later)
> 
...
>>>>> **u/AugSphere** [+1]  *Dark Lord of Corruption* (3 days later)
>>>>> 
...
>> **u/derefr** [+1]  (15 days later)
>> 
>> Sounds like *Down and Out in the Magic Kingdom* crossed with [DataPacRat's S.I.](http://www.datapacrat.com/SI/).
...
>> **u/OutOfNiceUsernames** [+6]  *fear of last pages* (8 hours later)
>> 
...
> **u/PeridexisErrant** [-4]  *put aside fear for courage, and death for life* (44 minutes later)
> 
> Readers: please note that posts should be about existing works of rational fiction - rather than abstract discussion.

>> **u/ToaKraka** [+13]  *https://i.imgur.com/OQGHleQ.png* (3 hours later)
>> 
>> What about [my zillions of links to *Critical Hits*](http://np.reddit.com/domain/critical-hits.com/), of which many have been "abstract discussion"? And this post *does* explicitly reference several existing works.
>> 
>> You (plural) could at least add [these spur-of-the-moment "rules"](http://np.reddit.com/r/rational/comments/4v9tx0/dc_the_blistering_stupidity_of_fallout_3/d5wvztt/) to the sidebar as soon as you decide to institute them.

>> **u/veruchai** [+13]  (7 hours later)
>> 
...
>> **u/Zephyr1011** [+13]  *Potentially Unfriendly Aspiring Divinity* (9 hours later)
>> 
