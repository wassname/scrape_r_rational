## Which should I read first: Bitter v. How to Avoid Death on a Daily Basis?

* Author: u/None *
* URL: 
* Score: 10

* Created: 2018-03-02T18:17:45

### Post:

[deleted]

### Comments:

> **u/bubba3737** [+15]  (an hour later)
> 
> HTADDB is about 2/3 snark and digressions. Bitter is more straightforward plot. HTADDB is much longer and is rated higher on topwebfiction. Read Metaworld Chronicles on RoyalRoadL instead

> **u/Fredlage** [+4]  (5 hours later)
> 
> It's worth noting that Bitter is very slow in its progression. Although he posts a new chapter daily, each is quite small. 
> Personally I like HTADDB better.

> **u/Lightwavers** [+6]  * s̮̹̃rͭ͆̄͊̓̍ͪ͝e̮̹̜͈ͫ̓̀̋̂v̥̭̻̖̗͕̓ͫ̎ͦa̵͇ͥ͆ͣ͐w̞͎̩̻̮̏̆̈́̅͂t͕̝̼͒̂͗͂h̋̿* (a day later)
> 
> [DELETED]  ^^^^^^^^^^^^^^^^0.412979935

>> **u/None** [+2]  (11 days later)
>> 
>> [deleted]

>>> **u/Lightwavers** [+1]  * s̮̹̃rͭ͆̄͊̓̍ͪ͝e̮̹̜͈ͫ̓̀̋̂v̥̭̻̖̗͕̓ͫ̎ͦa̵͇ͥ͆ͣ͐w̞͎̩̻̮̏̆̈́̅͂t͕̝̼͒̂͗͂h̋̿* (11 days later)
>>> 
>>> [DELETED]  ^^^^^^^^^^^^^^^^0.507394869

>> **u/wren42** [+1]  (6 days later)
>> 
>> ten?? but hardly anything has happened, that's still pretty much intro.  are you saying it doesn't go anywhere at all?

>>> **u/Lightwavers** [+3]  * s̮̹̃rͭ͆̄͊̓̍ͪ͝e̮̹̜͈ͫ̓̀̋̂v̥̭̻̖̗͕̓ͫ̎ͦa̵͇ͥ͆ͣ͐w̞͎̩̻̮̏̆̈́̅͂t͕̝̼͒̂͗͂h̋̿* (6 days later)
>>> 
>>> [DELETED]  ^^^^^^^^^^^^^^^^0.815106957

> **u/Permash** [+3]  (15 hours later)
> 
> They’re not too close to TGS, but they’re both great in their own way. I’d go with HTADDB, just for the snark and ridiculous plot.

> **u/serge_cell** [+3]  (a day later)
> 
> Bitter is definitely better IMO. I stopped following HTADDB then stuck in plot problem "protagonist become too strong". The only solution for such problem is an endless escalation, which become kind of cliche.

>> **u/None** [+1]  (a day later)
>> 
>> not the way moody is going with HTADDB...
>> honestly I think most powers could be equally badass and it wouldn't break the world if some new summoned hero gets an awesome power (or an anti colin power) and tries to kill colin
>> still I think too much random stuff is happening and the groundhog day power is not explained
>> also the last arc was confusing as hell for the protagonist and me and the batman gambits where just too much for me

> **u/Charlie___** [+2]  (2 hours later)
> 
> I felt like neither of those were nearly as good, but of the two Bitter is more like TGS. He also has some short stories about a magical thief that you can dig up various places, those were good.

>> **u/None** [+3]  (a day later)
>> 
>> on moody's website should be all:
>> http://moodylit.com/grin-the-cheat 
>> those are fun but short

---

