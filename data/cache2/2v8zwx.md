## [FF][RST] The Waves Arisen [Ch.15]

* Author: u/Wertifloke *
* URL: https://wertifloke.wordpress.com/2015/02/08/the-waves-arisen-chapter-15/
* Score: 28

* Created: 2015-02-09T01:01:34

### Post:

[Link to content](https://wertifloke.wordpress.com/2015/02/08/the-waves-arisen-chapter-15/)
...
>> [Wikipedia](https://en.wikipedia.org/wiki/Four_color_theorem#Generalizations) shows that this could be done with the surface of a torus; simply giving this depth away from the surface would lead to a contact area for every element with every other.
...
>>> 
>>> [Rope weapons are an actual thing](http://www.google.com/search?q=rope+weapon&oq=rope+weapon&aqs=chrome..69i57j0l5.5000j0j7&sourceid=chrome&es_sm=93&ie=UTF-8&gws_rd=ssl), so If I were Naruto, I'd try something like a braided cable, so that each element mingles with the others in turn.
>>> Edit:
>>> Naruto's best technique now creates something that looks and behaves like a wet pool noodle, so that's my new favorite mental image.
>>> 
...
>>> ######&#009;
>>> ####&#009;
>>> Section 8. [**Generalizations**](https://en.wikipedia.org/wiki/Four_color_theorem#Generalizations) of article  [**Four color theorem**](https://en.wikipedia.org/wiki/Four%20color%20theorem): [](#sfw) 
>>> ---
>>> >The four-color theorem applies not only to finite planar graphs, but also to [infinite graphs](https://en.wikipedia.org/wiki/Infinite_graph) that can be drawn without crossings in the plane, and even more generally to infinite graphs (possibly with an uncountable number of vertices) for which every finite subgraph is planar. To prove this, one can combine a proof of the theorem for finite planar graphs with the [De Bruijn–Erdős theorem](https://en.wikipedia.org/wiki/De_Bruijn%E2%80%93Erd%C5%91s_theorem_(graph_theory\)) stating that, if every finite subgraph of an infinite graph is *k*-colorable, then the whole graph is also *k*-colorable Nash-Williams (1967). This can also be seen as an immediate consequence of [Kurt Gödel](https://en.wikipedia.org/wiki/Kurt_G%C3%B6del)'s [compactness theorem](https://en.wikipedia.org/wiki/Compactness_theorem) for [first-order logic](https://en.wikipedia.org/wiki/First-order_logic), simply by expressing the colorability of an infinite graph with a set of logical formulae.
>>> >
>>> ---
>>> ^Interesting: [^Conjecture](https://en.wikipedia.org/wiki/Conjecture) ^| [^Georges ^Gonthier](https://en.wikipedia.org/wiki/Georges_Gonthier) ^| [^Kenneth ^Appel](https://en.wikipedia.org/wiki/Kenneth_Appel) ^| [^Coq](https://en.wikipedia.org/wiki/Coq) 
>>> ^Parent ^commenter ^can [^toggle ^NSFW](/message/compose?to=autowikibot&subject=AutoWikibot NSFW toggle&message=%2Btoggle-nsfw+cofjlzs) ^or[](#or) [^delete](/message/compose?to=autowikibot&subject=AutoWikibot Deletion&message=%2Bdelete+cofjlzs)^. ^Will ^also ^delete ^on ^comment ^score ^of ^-1 ^or ^less. ^| [^(FAQs)](http://www.np.reddit.com/r/autowikibot/wiki/index) ^| [^Mods](http://www.np.reddit.com/r/autowikibot/comments/1x013o/for_moderators_switches_commands_and_css/) ^| [^Magic ^Words](http://www.np.reddit.com/r/autowikibot/comments/1ux484/ask_wikibot/)
>>> 
